...
import FieldComms from '@/components/monitor/FieldComms';
      ...
      <div className="space-y-6">
        <div className="space-y-2">
          ...
          <h1 className="text-3xl md:text-4xl font-black tracking-tighter uppercase italic text-white leading-none">
            {incident.category === 'adversary_profile' && <span className="text-red-600">RSS OPERATIVE: </span>}{incident.title}
          </h1>
        ...
        <div className="relative group">
          ...
          <div className="relative bg-[#050606] border border-zinc-800/80 rounded-2xl p-6 md:p-8 space-y-4">
            <h2 className="text-[10px] font-black text-red-500 uppercase tracking-[0.4em] mb-4 flex items-center gap-2">
              <span className="w-1.5 h-4 bg-red-600 rounded-full" /> {incident.category === 'adversary_profile' ? 'Operative Dossier' : 'Intelligence Summary'}
            </h2>
            <p className="text-sm md:text-base text-zinc-300 leading-relaxed font-medium whitespace-pre-line italic">
              {incident.category === 'adversary_profile' ? `[DOSSIER LOG] ${incident.description}` : incident.description}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <TacticalMetric label={incident.category === 'adversary_profile' ? "Primary Activity Zone" : "Operational Zone"} value={incident.landmark || "UNSPECIFIED"} sub={incident.locationId ? OKHLA_AREAS[incident.locationId as OkhlaArea]?.label : "UNKNOWN SECTOR"} />
          <TacticalMetric 
          ...
          />
          <TacticalMetric label="Intel Classification" value={category.label.toUpperCase()} sub={`THREAT PROTOCOL: ${incident.severity.toUpperCase()}`} />
        ...
        <div className="flex flex-col sm:flex-row gap-4 pt-6">
          ...
          <div className="flex gap-2">
            <button 
              onClick={() => {
                const burnerUrl = `${window.location.origin}/burner/${incident.$id}`;
                navigator.clipboard.writeText(burnerUrl);
                addToast({ type: 'success', message: 'BURNER LINK COPIED TO CLIPBOARD' });
              }}
              className="px-6 py-4 bg-red-950/30 border border-red-900/50 text-red-500 text-[10px] font-black uppercase tracking-[0.3em] rounded-xl hover:bg-red-900/50 transition-all active:scale-95"
              title="Generate a self-destructing 24-hour link"
            >
              🔥 Burner
            </button>
            <button 
              onClick={() => { navigator.share?.({ title: incident.title, text: incident.description, url: window.location.href }); }}
              className="flex-1 px-10 py-4 bg-zinc-900 border border-zinc-800 text-zinc-400 text-[10px] font-black uppercase tracking-[0.3em] rounded-xl hover:border-zinc-600 hover:text-zinc-200 transition-all active:scale-95"
            >
              Broadcast
            </button>
          </div>
        ...
        {/* Encrypted Field Comms */}
        <FieldComms incidentId={incident.$id} />
      </div>
    </div>
  );
}
